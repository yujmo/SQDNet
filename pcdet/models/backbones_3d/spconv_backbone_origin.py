...
def index2uv(indices, batch_size, calib, stride, x_trans_train, trans_param):
    ...
    new_uv = indices.new(size=(indices.shape[0], 3)) # new方法创建的张量默认不会初始化其内容。new_uv中的值是内存中的未初始化值，是任意的。
...
def post_act_block_virconv(in_channels, out_channels, kernel_size, indice_key=None, stride=1, padding=0,
...
class NRConvBlock(nn.Module):
    ...
    def __init__(self, input_c=16, output_c=16, stride=1, padding=1, indice_key='vir1', conv_depth=False):
        ...
        self.stride = stride
        block = post_act_block_virconv
        block2d = post_act_block2d
        norm_fn = partial(nn.BatchNorm1d, eps=1e-3, momentum=0.01)
        self.conv_depth = conv_depth

        if self.stride > 1:
            self.down_layer = block(input_c,  # 3D voxels 降采样
                                    output_c,
                                    3,
                                    norm_fn=norm_fn,
                                    stride=stride,
...
class VirConvL8x(nn.Module):
    def __init__(self, model_cfg, input_channels, grid_size,  **kwargs):
        ...
        # print('spconv_backbone.py line:612')
        self.out_features = 128  # 64
        # self.out_features = model_cfg.OUT_FEATURES # 64
        ...
        self.sparse_shape = grid_size[::-1] + [1, 0, 0]
        # print('spconv_backbone.py line:620')
        input_channels = 8
        ...
        self.layer_discard_rate = 0.1
        last_pad = 0
        last_pad = self.model_cfg.get('last_pad', last_pad)
        self.conv_out = spconv.SparseSequential( #体素特征的输出
            # [200, 150, 5] -> [200, 150, 2]
