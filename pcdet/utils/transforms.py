...
class Resize(object):
    ...
    def __call__(self, img):
        ...
        img_type = img.dtype
        if img.ndim == 3:
            # img = skimage.transform.resize(img, self.size, order=0)
            # img = img.astype(img_type)
            # return img
            return np.array(Image.fromarray(img).resize(self.size, Image.NEAREST)).astype(img_type)
        elif img.ndim == 2:
            # img = skimage.transform.resize(img, self.size, order=0)
            # img = img.astype(img_type)
            # return img
...
class RandomCrop(object):
    ...
    def __call__(self, img):
        ...
        Returns:
            img (numpy.ndarray (C x H x W)): Cropped image.
        """
        x_l, x_r, y_b, y_t = self.get_params(img, self.crop_box)
